...
`sync` accepts the same configuration as [`CodePushOptions`](/dota/sdk/usage), except `checkFrequency` (not applicable to a one‑off call). Use it to override defaults for a specific run:

- `deploymentKey` (string) – see [Deployment Keys](/dota/sdk/usage#deployment-keys).
- `installMode` (codePush.InstallMode) – see [Install Modes](/dota/sdk/usage#install-modes).
- `mandatoryInstallMode` (codePush.InstallMode) – see [Mandatory Install Mode](/dota/sdk/usage#mandatory-install-mode).
- `minimumBackgroundDuration` (number) – see [Install Modes](/dota/sdk/usage#install-modes).
- `updateDialog` (UpdateDialogOptions) – see [Update Dialog](/dota/sdk/usage#update-dialog).
...
- Pairs with: [`disallowRestart`](#disallowrestart), [`restartApp`](#codepushrestartapp).
:::
...
- See also: [Deployment Keys](/dota/sdk/usage#deployment-keys), [Package objects → RemotePackage](#remotepackage).
:::
...
- Pairs with: [`allowRestart`](#codepushallowrestart), [`restartApp`](#codepushrestartapp), [Install Modes](/dota/sdk/usage#install-modes).
:::
...
- Pairs with: [Install Modes](/dota/sdk/usage#install-modes), [`disallowRestart`](#disallowrestart), [`allowRestart`](#allowrestart).
:::
